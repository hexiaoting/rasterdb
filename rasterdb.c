...
typedef struct RasterdbFdwExecutionState {
    ...
    bool eof_curfile_reached; /* true if last fetch reached EOF */
...
static void rasterdbBeginForeignScan(ForeignScanState *node, int eflags) {
    ...
    if(S_ISREG(s_buf.st_mode)) { 
        ...
        festate->rt_files = (char **) rtrealloc(festate->rt_files, sizeof(char *) * festate->rt_file_count);
...
//static int
//GetRasterBatch(char *filename, RTLOADERCFG *config, int cur_lineno, char **buf) {
//
//    //S3: analysis geotiff data to hexwkb
//    // Read config->batchsize lines offset cur_lineno from file to buf
//    return analysis_raster(filename, config, cur_lineno, buf);
//}
